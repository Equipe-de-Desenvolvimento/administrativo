...
class Inventario extends BaseController {
    ...
    function Inventario() {
        ...
        $this->load->model('estoque/inventario_model', 'inventario');
    ...
    function pesquisar($args = array()) {
        ...
        $this->loadView('estoque/inventario-lista', $args);

//            $this->carregarView($data);
    }

    function carregarinventario($estoque_entrada_id) {
    ...
    function excluir($estoque_entrada_id) {
        $valida = $this->inventario->excluir($estoque_entrada_id);
        if ($valida == 0) {
            $data['mensagem'] = 'Sucesso ao excluir a Inventario';
        } else {
    ...
    function gravar() {
        $exame_entrada_id = $this->inventario->gravar();
        if ($exame_entrada_id == "-1") {
            $data['mensagem'] = 'Erro ao gravar a Inventario. Opera&ccedil;&atilde;o cancelada.';
        } else {
            $data['mensagem'] = 'Sucesso ao gravar a Inventario.';
        }
        $this->session->set_flashdata('message', $data['mensagem']);
        redirect(base_url() . "estoque/inventario");
    }
