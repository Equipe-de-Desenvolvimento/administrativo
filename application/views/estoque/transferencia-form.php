...
//                $('#entrada').hide();
                ...
                $.getJSON('<?= base_url() ?>autocomplete/armazemtransferenciaentrada', {produto: $(this).val(), armazem: $("#armazem").val()}, function (j) {
                    ...
                    for (var i = 0; i < j.length; i++) {
                        ...
                        var data = new Date(j[i].validade);
                        var dataFormatada = ("0" + data.getDate()).substr(-2) + "/" + ("0" + (data.getMonth() + 1)).substr(-2) + "/" + data.getFullYear();
                        options += '<option value="' + j[i].estoque_entrada_id + '">QTDE: ' + j[i].total + '  Produto:  ' + j[i].descricao + ' Armazem:' + j[i].armazem + ' Lote: ' + j[i].lote + ' Validade: ' + dataFormatada + ' </option>';
                    }
